...
def fmt_avg_std(avg: float, sd: float, unit: str = "", width: int = 28) -> str:
    s = f"{avg:,.1f}{unit}" if not (sd and sd == sd) else f"{avg:,.1f}{unit} ± {sd:,.1f}{unit}"
    return f"{s:<{width}}"
...
RAW_TO_PRETTY = {
    ("C++", "Unordered map"): "[C++ unordered_map](https://cppreference.net/cpp/container/unordered_map.html)",
    ("C++", "Abseil flat map"): "[Abseil flat_map](https://abseil.io/docs/cpp/guides/container)",
    ("C++", "AxonCache"): "[AxonCache](https://github.com/AppLovin/AxonCache) C api",

    ("Golang", "Native go maps"): "[Go Map](https://pkg.go.dev/builtin#map)",
    ("Golang", "AxonCache"): "[AxonCache](https://github.com/AppLovin/AxonCache) Golang",
    ("Golang", "LMDB"): "[LMDB](https://symas.com/lmdb/)",
    ("Golang", "LevelDB"): "[LevelDB](https://github.com/syndtr/goleveldb) Pure Go version",
    ("Golang", "CDB"): "[CDB](https://cr.yp.to/cdb.html) Pure Go Version with mmap support",

    ("Python", "AxonCache"): "[AxonCache](https://github.com/AppLovin/AxonCache) Python",
    ("Python", "LMDB"): "[LMDB](https://github.com/jnwatson/py-lmdb/) Python module",
    ("Python", "Python CDB"): "[CDB](https://github.com/bbayles/python-pure-cdb) Pure Python module",
}
...
if __name__ == "__main__":
    ...
    for _ in range(runs):
        for cmd, runtime in cmds:
            for raw_label, ins_qps, look_qps, rt in run_and_parse_output_from_bench_cmd(cmd, runtime):
